...
class AAE_Model(object):
    def __init__(self, encoded_tweets, character_count=160, num_layers=4, kernel_size=3,
                scope='twitternlp_aae', batch_size=20, latent_size=30, epochs=1, clip=0.01):
        ...
        #builds network ops, training ops, clipping ops
        self.build_network()
    ...
    def build_network(self):
        ...
        #training ops
        encoder_optimizer = tf.train.RMSPropOptimizer(0.00005)
        generator_optimizer = tf.train.RMSPropOptimizer(0.0001)
    ...
    def encoder_loss(self):
        return -tf.reduce_mean(self.enc_critic_op)
    def critic_loss(self):
        wx = tf.reduce_mean(self.random_critic_op, 1)
        w_x = -tf.reduce_mean(self.enc_critic_op, 1)
        return tf.reduce_mean(tf.add(wx, w_x))
    def generation_loss(self):
        _x = self.enc_dec_op
        x = self.x
        #mse
        reconstruction_loss = tf.reduce_mean(tf.squared_difference(x, _x))
            ...
            try:
                ...
                for e in range(self.epochs):
                    ...
                    for b in range(self.num_batches):
                        n_critic = 80 if b+e*self.num_batches < 25 else self.n_critic
                        #start_time = time()
    ...
    def _decoder(self, z, reuse=True, scope="decoder"):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            #convert from z to conv_reduction_size for scaling
            net = tf.layers.dense(
                inputs=z,
            ...
            return tf.reshape(net, [-1, self.character_count])
